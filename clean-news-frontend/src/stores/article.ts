...
const getArticlesFromCache = (): Article[] | null => {
  ...
  const cacheRecord = JSON.parse(cache, parseConsideringDate) as CacheRecord<Article[]>
...
export const useArticles = (options?: SWRConfiguration): ApiHookResult<Article[]> => {
  const { data, error, mutate } = useSWR<Article[]>(['articles'], articlesFetcher, {
    ...options,
